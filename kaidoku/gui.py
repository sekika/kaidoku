...
class GUIBord(tkinter.Tk):
        ...
        for i in range(9):
            for j in range(9):
                if type(self.inputList[i][j]) != int:
                    if self.inputList[i][j].get() == '':
                        continue
                    self.move.append(str(i + 1) + str(j + 1) +
                                     self.inputList[i][j].get())
    ...
    def validation(self, P, idx, jdx):
        ...
        if len(P) > 1:
            return False
        if not (ord('1') <= ord(P) <= ord('9')):
            return False
        # If input value get stuck in a constraint , return False
        for i in range(9):
            if self.objToStr(self.inputList[idx][i]) == P:
                return False
            if self.objToStr(self.inputList[i][jdx]) == P:
                return False
            if self.objToStr(self.inputList[(idx//3)*3+i//3][(jdx//3)*3+i % 3]) == P:
                return False
        return True
